...
register_phys_mem(MEM_AREA_IO_NSEC, RPMH_PDC_COMPUTE_BASE, RPMH_PDC_COMPUTE_SIZE);
...
static TEE_Result lpass_setup(void)
{
	...
	res = qcom_clock_lucidevo_pll_enable(pll_base, &lpass_q6_pll_cfg);
	if (res != TEE_SUCCESS)
		return res;
			      ...
			      uint32_t gcc_aggre_axi_cbcr)
{
	...
	/* Turing slave-way bus clock branch, also connected to the NIU socket. */
	...
	/* Program the debug config register, then enable the core clock branch. */
	io_write32(pub + TURINGGDSP_QDSP6SS_DBG_CFG, 0);
	io_setbits32(core_cc + TURINGGDSP_QDSP6SS_CORE_CBCR,
		     CBCR_BRANCH_ENABLE_BIT);

	/* Configure and lock the Q6 PLL, then switch the core RCG onto it. */
	res = qcom_clock_lucidevo_pll_enable(pll_base, &gpdsp_q6_pll_cfg);
	if (res != TEE_SUCCESS)
		return res;
...
TEE_Result qcom_clock_enable_pas_processor(enum qcom_clk_group group)
{
	...
	case QCOM_CLKS_LPASS:
		/*
		 * LPASS configures its Q6 PLL and switches the core RCG during the
		 * setup stage (lpass_setup, before the boot FSM), unlike the Turing
		 * NSP. The core is released by BOOT_CORE_START in lpass_fw_start, so
...
struct cdsp_reset_regs {
	...
	uint32_t computess_restart_bit;	/* field within AOSS_CC_COMPUTESS_RESTART */
	uint32_t ret_cfg_settle_us;	/* settle delay after RET_CFG (CDSP0 only) */
};
...
static TEE_Result cdsp_reset_processor(const struct cdsp_reset_regs *r)
{
	struct io_pa_va turing_cc_io = { .pa = r->turing_base +
						TURINGNSP_CC_OFFSET };
	vaddr_t cc_base = io_pa_or_va(&turing_cc_io, TURINGNSP_PROC_WINDOW_SIZE);
	...
	if (io_read32(tcsr + r->tcsr_pwr_on) & TCSR_TURING_BIT) {
		if (!(io_read32(tcsr + r->tcsr_master_idle) & TCSR_TURING_BIT)) {
...
TEE_Result qcom_clock_pas_reset(enum qcom_clk_group group)
{
	...
	case QCOM_CLKS_LPASS:
		/*
		 * The LPASS subsystem reset (Reset_LPASSProcessor) is the SSR /
		 * tear-down path; cold bring-up brings the Q6 up from a known state
		 * and the LPASS core HM domain is already powered at TZ boot.
		 */
		return TEE_SUCCESS;
	default:
		return TEE_ERROR_NOT_SUPPORTED;
