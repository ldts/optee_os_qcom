...
TEE_Result gpdsp0_fw_start(void)
{
	struct qcom_pas_data *data = gpdsp0_get_pas_data();
...
TEE_Result gpdsp0_fw_shutdown(void)
{
	return qcom_clock_pas_reset(QCOM_CLKS_GPDSP0);
}

TEE_Result gpdsp0_get_resource_table(struct resource_table *rt,
				     size_t *rt_size)
{
...
struct qcom_pas_data *gpdsp0_get_pas_data(void)
{
